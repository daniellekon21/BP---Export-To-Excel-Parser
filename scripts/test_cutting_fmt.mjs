...
function printRecords(label, text) {
  ...
  for (const r of records) {
    console.log(
      `  ${String(r.cmNumber).padEnd(8)} | HC=${String(r.radialsHC ?? "-").padStart(3)} | LC=${String(r.radialsLC ?? "-").padStart(3)} | AGRI=${String(r.radialsAgri ?? "-").padStart(3)} | TREADS=${String(r.radialsAgriTreads ?? "-").padStart(3)}`
    );
  }
}
...
printRecords("Text 1 — HC+LC dual-line", TEXT1);
printRecords("Text 2 — HC/LC single-line", TEXT2);
printRecords("Text 3 — all machines dual-line", TEXT3);
printRecords("Text 4 — } typo as zero", TEXT4);
const TEXT6 = wrap(
`22/10/2025
11:00-12:00
CM1-HC=29
...
printRecords("Text 5 — CM3 dual-line, CM1/CM2 HC only", TEXT5);
const TEXT8 = wrap(
`31/10/2025
09:00-10:00
CM3-HC=19
...
printRecords("Text 6 — no parens, CM3 continuation line", TEXT6);
printRecords("Text 8 — bare LC=02 continuation", TEXT8);
printRecords("Text 9 — Machine X format", TEXT9);
printRecords("Text 7 — agri vs agri treads with parens", TEXT7);
printRecords("Text 10 — offloading + real data, expect 1 CM2 row only", TEXT10);
...
function printRecordsWithLog(label, text) {
  ...
  for (const r of records) {
    ...
    console.log(
      `  ${String(r.cmNumber).padEnd(8)} | HC=${String(r.radialsHC ?? "-").padStart(3)} | LC=${String(r.radialsLC ?? "-").padStart(3)} | AGRI=${String(r.radialsAgri ?? "-").padStart(3)} | TREADS=${String(r.radialsAgriTreads ?? "-").padStart(3)}${amb}`
    );
  }
